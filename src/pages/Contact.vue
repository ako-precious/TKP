...
import { Check, Mail, MapPin, MessageCircle, PackagePlus, Phone, X } from 'lucide-vue-next'
...
<template>
  ...
  <section class="px-4 py-16 sm:px-6 lg:px-8"><div class="mx-auto grid max-w-7xl gap-9 lg:grid-cols-[0.75fr_1.25fr]">
    <aside v-reveal class="rounded-3xl bg-forest p-7 text-cream sm:p-9"><p class="eyebrow text-maize">Come say hello</p><h2 class="mt-3 font-serif text-4xl font-black">We’re here to help.</h2><p class="mt-4 leading-7 text-cream/70">Ask about delivery, availability or the Kenyan favourite you’ve been missing.</p><div class="mt-8 space-y-5">
      <a v-reveal="{ variant: 'settle' }" href="mailto:memoirsbyk@outlook.com" class="contact-row"><Mail /><span><small>Email</small>memoirsbyk@outlook.com</span></a>
      <a v-reveal="{ variant: 'settle', delay: 0.07 }" href="tel:+16476796760" class="contact-row"><Phone /><span><small>Phone</small>647-679-6760</span></a>
      <a v-reveal="{ variant: 'settle', delay: 0.14 }" href="https://wa.me/17052849804" target="_blank" rel="noreferrer" class="contact-row whatsapp-row"><span class="whatsapp-ping"><MessageCircle /></span><span><small>WhatsApp ordering</small>705-284-9804</span></a>
      <div v-reveal="{ variant: 'settle', delay: 0.21 }" class="contact-row"><MapPin /><span><small>Service area</small>Barrie, Hamilton & the GTA</span></div>
    ...
    <form v-reveal="'100ms'" class="rounded-3xl border border-basket/15 bg-white p-7 shadow-sm sm:p-9" @submit.prevent="submitContact">
      <div v-if="sent" class="form-success rounded-2xl bg-maize/20 p-5 text-forest"><span class="success-check"><Check /></span><h2 class="mt-4 font-serif text-2xl font-black">Asante! Your message is ready.</h2><p class="mt-2 text-sm leading-6">This demo has validated your details. Connect the marked form API before launch to deliver submissions.</p></div>
  ...
  <Teleport to="body"><Transition name="request-modal"><div v-if="requestOpen" class="fixed inset-0 z-[70] grid place-items-center bg-forest/65 p-4 backdrop-blur-sm" @click.self="requestOpen = false"><form class="request-modal-card w-full max-w-lg rounded-3xl bg-cream p-7 shadow-2xl" @submit.prevent="submitRequest"><div class="flex items-start justify-between"><div><p class="eyebrow">Special request</p><h2 class="mt-2 font-serif text-3xl font-black text-forest">What should we stock?</h2></div><button class="icon-button" type="button" aria-label="Close request form" @click="requestOpen = false"><X /></button></div><div v-if="requestSent" class="mt-6 rounded-2xl bg-maize/20 p-5 text-forest"><strong>Request received — asante!</strong><p class="mt-1 text-sm">We’ll use your suggestion when planning future stock.</p></div><div v-else class="mt-6 grid gap-5"><label class="field-label">Product Name<input v-model.trim="request.product" class="form-field" required minlength="2" /></label><label class="field-label">Description / Why you want it<textarea v-model.trim="request.reason" class="form-field min-h-28" required minlength="8"></textarea></label><label class="field-label">Email <span class="font-normal text-charcoal/45">(optional)</span><input v-model.trim="request.email" class="form-field" type="email" /></label><button class="button-primary justify-center" type="submit">Send Product Request</button></div></form></div></Transition></Teleport>
</template>
